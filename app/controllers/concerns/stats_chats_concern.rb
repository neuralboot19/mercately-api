...
module StatsChatsConcern
  ...
  private
    ...
    def total_chats_answered_ws
      ...
      sql = 'select count(*) from (select t1.customer_id, t1.created_at, ' \
        'COALESCE(lag(t1.created_at) over(partition by t1.customer_id order by t1.created_at), ' \
        '?) prev_date from (select gsm1.customer_id, ' \
        "gsm1.created_at + '24 hours'::interval as created_at from #{@integration} gsm1 " \
        ...
        'order by gsm1.created_at) t1 where t1.created_at >= ? and t1.created_at <= ?) t2 ' \
        ...
        'gsm3.created_at > t2.prev_date and gsm3.created_at <= t2.created_at limit 1) is not null'
    ...
    def total_chats_answered_msn
      ...
      sql = 'select count(*) from (select t1.customer_id, t1.created_at, ' \
        ...
        "fm1.created_at + '24 hours'::interval as created_at from facebook_messages fm1 " \
        ...
        'order by fm1.created_at) t1 where t1.created_at >= ? and t1.created_at <= ?) t2 ' \
        ...
        'fm3.created_at > t2.prev_date and fm3.created_at <= t2.created_at limit 1) is not null'
