...
RSpec.describe KarixWhatsappMessage, type: :model do
  ...
  describe '#apply_cost' do
    let(:retailer) { create(:retailer, :karix_integrated, ws_balance: 10.0) }

    context 'when the message status is failed' do
      let(:message) { build(:karix_whatsapp_message, :outbound, :notification, retailer: retailer, status: 'failed') }
    ...
    context 'when the message status is not failed' do
      let(:message) { build(:karix_whatsapp_message, :outbound, :conversation, retailer: retailer, status: 'queued') }
      ...
      it 'assigns the retailer conversation/notification cost to the message' do
        ...
        expect(retailer.reload.ws_balance).to eq(10.0 - retailer.send("ws_#{message.message_type}_cost"))
      end
    end
  end
  ...
  describe '#set_sender_information' do
    ...
    let(:message) do
      build(:karix_whatsapp_message, :outbound, retailer: retailer, retailer_user: retailer_user)
    end
  ...
  describe '#insert_on_agent_queue' do
    ...
    let(:message1) { build(:karix_whatsapp_message, :inbound, retailer: retailer, customer: customer1) }
    let(:message2) { build(:karix_whatsapp_message, :inbound, retailer: retailer, customer: customer2) }
    let(:message3) { build(:karix_whatsapp_message, :inbound, retailer: retailer, customer: customer3) }
